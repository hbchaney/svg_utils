...
class ShapeManagerUnitTest(ut.TestCase): 
    ...
    def test_shape_manager(self) -> None: 
        for s in self.shapes: 
             self.shape_manager.add_and_compare_shape(s) 
        
        output_name = "test_shape_compare01.svg"
        ...
        max_x_list = [] 
        max_y_list = [] 
        for s in self.shape_manager.shape_list: 
...
def tests(): 
    suite = ut.TestSuite() 
    suite.addTest(ShapeManagerUnitTest("test_shape_manager"))
